...
void ExportLibWsjCppCliPy::export__init__py() {
    ...
    .sub("class FreeHackQuestClient:")
        ...
        .sub("def generate_base_command(self, method):")
            ...
            .end()
        .end().sub("")
        .sub("def __send_command(self, req):")
            .add("\"\"\"Send request\"\"\"")
    ...
    for (; it!=g_pCmdHandlers->end(); ++it) {
        ...
        */
        ...
        builder
            .sub("for param_name in req:")
                .sub("if param_name not in allowed_params:")
                    .add("raise Exception('Excess parameter \"' + param_name + '\" (lib)')")
                    .end()
                .end();
        for (int i = 0; i < vVin.size(); i++) {
            CmdInputDef inDef = vVin[i];
        ...
        builder
            .add("return self.__send_command(request_json)")
            .end()
        .end()
        .sub("");
    }
...
void ExportLibWsjCppCliPy::exportClientPy() {
    std::ofstream __init__;
    std::cout << " * write code to libfreehackquest-client-py/libfreehackquestclient/FreeHackQuestClient.py " << std::endl;
    __init__.open ("libfreehackquest-client-py/libfreehackquestclient/FreeHackQuestClient.py");
