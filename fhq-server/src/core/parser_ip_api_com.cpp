...
bool ParserIpApiCom::parse(const std::string &sIpAddress, const std::string &sJson) {
    ...
    if (!nlohmann::json::accept(sJson)) {
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + ", Expected json: " + sJson);
        return false;
    }
    ...
    try {
        ...
        if (sStatus == "fail") {
            WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> status: fail " + sJson);
            return false;
        }
    } catch (const std::exception &e) {
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'status' in struct \n" + sJson + "\n" + std::string(e.what()));
        return false;
    }
    ...
    try {
        ...
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'country' in struct \n" + sJson + "\n" + std::string(e.what()));
        m_sCountry = "";
    }
    ...
    try {
        ...
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'regionName' in struct \n" + sJson + "\n" + std::string(e.what()));
        m_sRegionName = "";
    }
    ...
    try {
        ...
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'city' in struct \n" + sJson + "\n" + std::string(e.what()));
        m_sCity = "";
    }
    ...
    try {
        ...
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'lat' in struct \n" + sJson + "\n" + std::string(e.what()));
        m_nLat = 0.0;
    }
    ...
    try {
        ...
        WSJCppLog::err(TAG, "ip-api.com/json/" + sIpAddress + " -> wrong field 'lon' in struct \n" + sJson + "\n" + std::string(e.what()));
        m_nLon = 0.0;
    }
