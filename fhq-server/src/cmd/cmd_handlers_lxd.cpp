...
void CmdHandlerLXDList::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployOrchestra *pOrchestra = findWsjcppEmploy<EmployOrchestra>();
    if (!pOrchestra->initConnection()) {
        pRequest->fail(WsjcppJsonRpc20Error(500, pOrchestra->lastError()));
        return;
    }
...
void CmdHandlerLXDImportService::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (!nlohmann::json::accept(sConfig)) {
        pRequest->fail(WsjcppJsonRpc20Error(400, "Json string isn't valid."));
    }
    ...
    if (sName.empty()) {
        if (jsonConfig.find("name") != jsonConfig.end()) {
            ...
            pRequest->fail(WsjcppJsonRpc20Error(400, "Container name not found."));
        }
...
void CmdHandlerLXDImportServiceFromZip::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (zip.open(QuaZip::mdUnzip)) {
        ...
        if (zip.getZipError() != UNZ_OK) {
            pRequest->fail(WsjcppJsonRpc20Error(400, "Cant unzip " + std::to_string(zip.getZipError())));
            return;
        } else {
    ...
    if (sConfig.empty()) {
        pRequest->fail(WsjcppJsonRpc20Error(nErrorCode, "Not found service.json in zip archive."));
        zip.close();
        return;
    }

    if (!nlohmann::json::accept(sConfig)) {
        pRequest->fail(WsjcppJsonRpc20Error(400, "Service json file isn't valid."));
        zip.close();
        return;
    }
    nlohmann::json jsonConfig = nlohmann::json::parse(sConfig);
    ...
    if (!pOrchestra->create_service(jsonConfig, sError)) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant create service. Error: " + sError));
        zip.close();
        return;
    }
    ...
    if (!pOrchestra->find_service(jsonConfig["name"].get<std::string>(), service)) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant find service. Error: " + pOrchestra->lastError()));
        zip.close();
        return;
    }
    ...
    if (container->get_status() != "Running" && !container->start()) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant start container. Error: " + container->get_error()));
        zip.close();
        return;
    }

    if (!container->exec("mkdir -p /root/service")) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant create service directory. Error: " + container->get_error()));
        zip.close();
        return;
    }
    ...
    for (bool more = zip.goToFirstFile(); more; more = zip.goToNextFile()) {
        ...
        if (!file_name.empty() && file_name != "service.json") {
            ...
            if (!container->push_file("/root/service/" + file_name, file.readAll().toStdString())) {
                pRequest->fail(WsjcppJsonRpc20Error(nErrorCode, container->get_error()));
                file.close();
                zip.close();
                return;
            }
    ...
    if (!service->build()) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant build service. Error: " + service->get_error()));
        return;
    }
...
void CmdHandlerLXDStartService::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (!service->start()) {
        pRequest->fail(WsjcppJsonRpc20Error(500, "Cant start service. Error: " + service->get_error()));
        return;
    }
