...
void CmdClassbookAddRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (nParentID != 0) {
        ...
        query.exec();
        if (!query.next()) {
            pRequest->fail(WsjcppJsonRpc20Error(404, "Not found article with this id"));
            return;
        }
    }
    ...
    if (sUuid != "") {
        ...
        if (query.next()) {
            pRequest->fail(WsjcppJsonRpc20Error(403, "Uuid already exist"));
            return;
        }
    } else {
...
void CmdClassbookDeleteRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    ...
    if (nClassbookID !=0) {
        ...
        if (query.next()) {
            pRequest->fail(WsjcppJsonRpc20Error(403, "Could not delete, because childs exists. Please remove childs first."));
            return;
        }
...
void CmdClassbookExportHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    ...
    if (sOutput != "html" && sOutput != "markdown") {
        pRequest->fail(WsjcppJsonRpc20Error(403, "The output is not supported."));
        return;
    }
    QList<QString> langs;
    langs << "en" << "de" << "ru"; // TODO move to basic support employ
    if (!langs.contains(QString::fromStdString(sLang))) {
        pRequest->fail(WsjcppJsonRpc20Error(403, "The language is not supported."));
        return;
    }
...
void CmdClassbookInfoHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    ...
    if (query.next()) {
        ...
        pRequest->fail(WsjcppJsonRpc20Error(404, "Not found the article"));
        return;
    }
    ...
    if (jsonRequest.find("lang") != jsonRequest.end()) {
        ...
        if (!allow_lang.contains(QString::fromStdString(sLang))) {
            pRequest->fail(WsjcppJsonRpc20Error(404, "Language is not support"));
            return;
        }
    } else {
        sLang = "en";
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (query.next()) {
            ...
            pRequest->fail(WsjcppJsonRpc20Error(404, "Error in PATHFINDER. Not found the article with a given classbookid"));
            return;
        }
    }
...
void CmdClassbookListHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    nlohmann::json jsonResponse;
    ...
    if (jsonRequest.find("search") != jsonRequest.end()) {
        ...
        if (!query1.exec()) {
            pRequest->fail(WsjcppJsonRpc20Error(500, query1.lastError().text().toStdString()));
            return;
        }
        while (query1.next()) {
        ...
        if (!query.next() && nParentID != 0) {
            pRequest->fail(WsjcppJsonRpc20Error(404, "Not found the article with a given parentid"));
            return;
        }
...
void CmdClassbookUpdateRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    ...
    if (nClassbookID == 0) {
        pRequest->fail(WsjcppJsonRpc20Error(403, "Not today. It's root article id"));
        return;
    }
    ...
    if (!query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(404, "Not found a article with a given classbookid"));
        return;
    }
    ...
    if (jsonRequest.find("name") == jsonRequest.end()
        ...
        pRequest->fail(WsjcppJsonRpc20Error(403, "Not found a charges. Not enough parameters"));
        return;
    }
    ...
    if (jsonRequest.find("parentid") != jsonRequest.end()) {
        ...
        if (nParentID != 0) {
            ...
            if (!query.next()) {
                pRequest->fail(WsjcppJsonRpc20Error(404, "Not found a article with a given parentid"));
                return;
            }
        }
    ...
    if (query.next()) {
        ...
        pRequest->fail(WsjcppJsonRpc20Error(404, "Not found article"));
        return;
    }
...
void CmdClassbookLocalizationAddRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    nlohmann::json jsonResponse;
    ...
    if (query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(400, "This lang already exist"));
        return;
    }
...
void CmdClassbookLocalizationDeleteRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    ...
    if (!query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(404, "This localization doesn't exist"));
        return;
    }
...
void CmdClassbookLocalizationInfoHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
...
void CmdClassbookLocalizationUpdateRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
...
void CmdClassbookProposalAddRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    ...
    if (!query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(404, "This article or localization doesn't exist"));
        return;
    }
    QSqlRecord record = query.record();
...
void CmdClassbookProposalDeleteRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    ...
    if (!query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(404, "This proposal doesn't exist"));
        return;
    }
...
void CmdClassbookProposalInfoHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
...
void CmdClassbookProposalListHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    nlohmann::json jsonResponse;
    ...
    if (jsonRequest.find("classbookid") != jsonRequest.end()) {
        ...
        if (!query.next()) {
            pRequest->fail(WsjcppJsonRpc20Error(404, "This article doesn't exist"));
            return;
        }
...
void CmdClassbookProposalPrepareMergeRecordHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
...
void CmdClassbookProposalApproveHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
...
void CmdClassbookProposalUpdateHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
...
void CmdClassbookHandler::handle(WsjcppJsonRpc20Request *pRequest) {
    EmployDatabase *pDatabase = findWsjcppEmploy<EmployDatabase>();
    nlohmann::json jsonRequest = pRequest->jsonRequest();
    nlohmann::json jsonResponse;
