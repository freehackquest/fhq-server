...
void CmdHandlerUsefulLinksUserFavorite::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(403, "Already exists"));
        return;
    }
...
void CmdHandlerUsefulLinksCommentAdd::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (sCommentValue == "") {
        pRequest->fail(WsjcppJsonRpc20Error(400, "comment could not be empty"));
        return;
    }

    if (sCommentValue.length() > 1024) {
        pRequest->fail(WsjcppJsonRpc20Error(400, "comment must be less then < 1024"));
        return;
    }
...
void CmdHandlerUsefulLinksCommentDelete::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (query.next()) {
        ...
        pRequest->fail(WsjcppJsonRpc20Error(404, "COMMENT_NOT_FOUND"));
        return;
    }
...
void CmdHandlerUsefulLinksTagAdd::handle(WsjcppJsonRpc20Request *pRequest) {
    ...
    if (sTagValue == "") {
        pRequest->fail(WsjcppJsonRpc20Error(400, "tag could not be empty"));
        return;
    }
    ...
    if (query.next()) {
        pRequest->fail(WsjcppJsonRpc20Error(400, "TAG_ALREADY_EXISTS"));
        return;
    } else {
