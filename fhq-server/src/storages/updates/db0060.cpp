...
bool StorageStruct0060::apply(Storage *pStorage, std::string &error){
	...
	email_delivery.addColumn(StorageStructColumn("id")
		.number().autoIncrement().primaryKey().notNull());

	email_delivery.addColumn(StorageStructColumn("to_email")
		.string(255).notNull());

	email_delivery.addColumn(StorageStructColumn("subject")
		.string(255).notNull());

	email_delivery.addColumn(StorageStructColumn("message")
		.text().notNull());

	email_delivery.addColumn(StorageStructColumn("priority")
		.string(255).notNull());

	email_delivery.addColumn(StorageStructColumn("status")
		.string(255).notNull());
	
	email_delivery.addColumn(StorageStructColumn("dt")
		.datetime().notNull());

	pStorage->applyStruct(email_delivery);
