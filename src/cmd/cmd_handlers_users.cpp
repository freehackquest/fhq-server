...
CmdHandlerUsersScoreboard::CmdHandlerUsersScoreboard() : CmdHandlerBase("scoreboard", "Method return scoreboard") {
...
CmdHandlerGetMap::CmdHandlerGetMap() : CmdHandlerBase("getmap", "Returned coordinate list") {
...
void CmdHandlerGetMap::handle(ModelRequest *pRequest) {
  ...
  QSqlQuery query(db);
  query.prepare("SELECT COUNT(*) as cnt, latitude, longitude FROM `users` "
...
CmdHandlerLogin::CmdHandlerLogin() : CmdHandlerBase("login", "Method for login") {
...
void CmdHandlerLogin::handle(ModelRequest *pRequest) {
  ...
  if (query.next()) {
    ...
    query_token.prepare("INSERT INTO users_tokens (userid, token, status, data, start_date, "
                        "end_date, start_dt, end_dt) VALUES(:userid, :token, :status, :data, "
                        "NOW(), NOW() + INTERVAL 1 DAY, :start_dt, :end_dt)");
    ...
    query_token.bindValue(":end_dt",
                          (long long)(nStartDate + 86400000)); // + 24 hours // TODO must be configurable
...
CmdHandlerRegistration::CmdHandlerRegistration() : CmdHandlerBase("registration", "Method for registration") {
  ...
  requireStringParam("email", "E-mail").addValidator(new WsjcppValidatorEmail());
...
CmdHandlerToken::CmdHandlerToken() : CmdHandlerBase("token", "Method for login by token") {
...
CmdHandlerUsersAdd::CmdHandlerUsersAdd() : CmdHandlerBase("users_add", "Method for add new user") {
  ...
  // validation and description input fields
  requireStringParam("uuid", "User's Global Unique Identifier").addValidator(new WsjcppValidatorUUID());

  requireStringParam("email", "User's E-mail");
  requireStringParam("nick", "User's nick").addValidator(new WsjcppValidatorStringLength(4, 127));
  requireStringParam("password", "Password").addValidator(new WsjcppValidatorStringLength(4, 127));
...
CmdHandlerUser::CmdHandlerUser() : CmdHandlerBase("user", "Return user info") {
...
void CmdHandlerUser::handle(ModelRequest *pRequest) {
  ...
  {
    QSqlQuery query(db);
    query.prepare("SELECT name, value, date_change FROM users_profile WHERE "
                  "userid = :userid");
    query.bindValue(":userid", nUserID);
    if (!query.exec()) {
      pRequest->sendMessageError(cmd(), WsjcppJsonRpc20Error(500, query.lastError().text().toStdString()));
...
CmdHandlerUsersInfo::CmdHandlerUsersInfo() : CmdHandlerBase("users_info", "Return user info") {
...
CmdHandlerUserSkills::CmdHandlerUserSkills() : CmdHandlerBase("user_skills", "Return user skills info") {
...
void CmdHandlerUserSkills::handle(ModelRequest *pRequest) {
  ...
  {
    QSqlQuery query(db);
    query.prepare("SELECT q.subject, sum(q.score) as sum_subject FROM quest q WHERE ! "
                  "ISNULL( q.subject ) AND (q.state = 'open') GROUP BY q.subject");
    if (!query.exec()) {
      pRequest->sendMessageError(cmd(), WsjcppJsonRpc20Error(500, query.lastError().text().toStdString()));
      return;
    };
  ...
  {
    QSqlQuery query(db);
    query.prepare("SELECT uq.userid, q.subject, SUM( q.score ) as sum_score "
                  "FROM users_quests uq INNER JOIN quest q ON uq.questid = "
                  "q.idquest WHERE ! ISNULL( q.subject ) AND uq.userid = "
                  ":userid GROUP BY uq.userid, q.subject");
    query.bindValue(":userid", nUserID);
    if (!query.exec()) {
      pRequest->sendMessageError(cmd(), WsjcppJsonRpc20Error(500, query.lastError().text().toStdString()));
      return;
...
CmdHandlerUserUpdate::CmdHandlerUserUpdate() : CmdHandlerBase("user_update", "Update user info") {
...
CmdHandlerUsersDelete::CmdHandlerUsersDelete() : CmdHandlerBase("users_delete", "Method for deleting a user") {
...
CmdHandlerUsers::CmdHandlerUsers() : CmdHandlerBase("users", "Method return list of users") {
  ...
  optionalStringParam("filter_role",
                      "Filter by user role"); // TODO validator role
...
CmdHandlerUsersChangeEmail::CmdHandlerUsersChangeEmail()
  ...
  // validation and description input fields
  requireStringParam("email", "New E-mail").addValidator(new WsjcppValidatorEmail());
...
CmdHandlerUsersTokens::CmdHandlerUsersTokens() : CmdHandlerBase("users_tokens", "List of active user tokens") {
...
void CmdHandlerUsersTokensDelete::handle(ModelRequest *pRequest) {
  ...
  QSqlDatabase db = *(pDatabase->database());
  QSqlQuery query(db);
  query.prepare("SELECT * FROM users_tokens WHERE userid = :userid AND id = "
                ":tokenid ORDER BY id DESC");
  query.bindValue(":userid", nUserID);
