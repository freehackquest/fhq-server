...
// ---------------------------------------------------------------------
// Any actions with the container. Actions: create, start, stop and delete
// container

REGISTRY_CMD(CmdHandlerLXDContainers)

CmdHandlerLXDContainers::CmdHandlerLXDContainers()
    : CmdHandlerBase("lxd_containers", "Any actions with the container. Actions: create, start, "
  ...
  requireStringParam("action",
                     "Actions: create, start, stop and delete container"); // TODO validator
}
...
void CmdHandlerLXDContainers::handle(ModelRequest *pRequest) {
  ...
  if (std::find(actions.begin(), actions.end(), action) == actions.end()) {
    sError = "Non-existent operation. Possible operations: create, start, "
             "stop, delete.";
    nErrorCode = 404;
  }
...
CmdHandlerLXDInfo::CmdHandlerLXDInfo() : CmdHandlerBase("lxd_info", "Get information about the orhestra, containers.") {
...
bool CmdHandlerLXDInfo::get_state(const std::string &sName, std::string &sError, int &nErrorCode,
                                  nlohmann::json &jsonState) {
  EmployOrchestra *pOrchestra = findWsjcppEmploy<EmployOrchestra>();
  if (!pOrchestra->initConnection()) {
    return false;
...
CmdHandlerLXDList::CmdHandlerLXDList() : CmdHandlerBase("lxd_list", "Get information about all containers.") {
...
CmdHandlerLXDExec::CmdHandlerLXDExec() : CmdHandlerBase("lxd_exec", "Exec command in the container with name.") {
...
CmdHandlerLXDFile::CmdHandlerLXDFile() : CmdHandlerBase("lxd_file", "Pull, push, delete file inside the container.") {
...
CmdHandlerLXDOpenPort::CmdHandlerLXDOpenPort() : CmdHandlerBase("lxd_open_port", "Opens the container port.") {
...
void CmdHandlerLXDImportServiceFromZip::handle(ModelRequest *pRequest) {
  ...
  if (!pOrchestra->find_service(jsonConfig["name"].get<std::string>(), service)) {
    pRequest->sendMessageError(cmd(),
                               WsjcppJsonRpc20Error(500, "Cant find service. Error: " + pOrchestra->lastError()));
    zip.close();
    return;
  }
  ...
  if (container->get_status() != "Running" && !container->start()) {
    pRequest->sendMessageError(cmd(),
                               WsjcppJsonRpc20Error(500, "Cant start container. Error: " + container->get_error()));
    zip.close();
    return;
  }
...
CmdHandlerLXDStartService::CmdHandlerLXDStartService() : CmdHandlerBase("lxd_start_service", "Start service.") {
